...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None: # ghost is in jail and its probability is 1.0
            updatedBeliefs[self.getJailPosition()] = 1.0
            self.beliefs = updatedBeliefs
            return

        for ghostPosition in self.legalPositions:
            # updating belief based on observation:
            # updatedBelief = P(observation | trueDistances) * previousBelief
            trueDistance = util.manhattanDistance(ghostPosition, pacmanPosition)
            updatedBeliefs[ghostPosition] = emissionModel[trueDistance] * self.beliefs[ghostPosition]

        updatedBeliefs.normalize() # values to indicate probability
        self.beliefs = updatedBeliefs
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        updatedBeliefs = util.Counter()
        for ghostPosition in self.legalPositions:
            # next action distribution for ghost located ad ghostPos
            newGhostPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, ghostPosition))
            for newPos, prob in newGhostPosDist.items():
                # updatedBelief = probability of action taking us to newPos * belief of ghost being in position from which the action was taken
                updatedBeliefs[newPos] += prob * self.beliefs[ghostPosition]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        numParticles = 0
        while numParticles < self.numParticles:
            for pos in self.legalPositions: # round robbin
                if numParticles < self.numParticles:
                    self.particles.append(pos)
                    numParticles += 1

        random.shuffle(self.particles) # shuffling the order of particles
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None: # ghost is in jail and its probability is 1.0
            # all particles should end up in jailPosition
        ...
        for ghostPosition in self.legalPositions:
            ...
            # updatedBelief = P(observation | trueDistances) * previousBelief
            trueDistance = util.manhattanDistance(ghostPosition, pacmanPosition)
            updatedBeliefs[ghostPosition] = emissionModel[trueDistance] * oldBeliefs[ghostPosition]

        if all(v == 0 for v in updatedBeliefs.values()): # weights of particles are all 0
            self.initializeUniformly(gameState)
        else: # resample
            self.particles = util.nSample(updatedBeliefs.values(), updatedBeliefs.keys(), self.numParticles)
    ...
    def elapseTime(self, gameState):
        ...
        """
        # updating particles:
        # for each particle get sample from ghostPosDistribution
        self.particles = map(lambda ghostPos: util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, ghostPos))), self.particles)
    ...
    def getBeliefDistribution(self):
        ...
        beliefs.incrementAll(self.particles, 1)
        beliefs.normalize()
        return beliefs
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        numParticles = 0
        while numParticles < self.numParticles:
            for pos in particlePositions: # round robbin
                if numParticles < self.numParticles:
                    self.particles.append(pos)
                    numParticles += 1
    ...
    def observeState(self, gameState):
        ...
        likelihoods = util.Counter()

        for particle in self.particles:
            likelihood = 1 # weight of entire sample (particle)
            for i in range(self.numGhosts):
                if noisyDistances[i] is None: # i-th ghost is definately in jail
                    particle = self.getParticleWithGhostInJail(particle, i)
                else:
                    trueDistance = util.manhattanDistance(particle[i], pacmanPosition)
                    likelihood *= emissionModels[i][trueDistance] # update weight of sample
            likelihoods[particle] += likelihood # likelihood of state is weighted with all samples (cummulative)

        if all(v == 0 for v in likelihoods.values()): # weights of particles are all 0
            self.initializeParticles()
            for index, particle in enumerate(self.particles):
                for i in range(self.numGhosts):
                    if noisyDistances[i] is None: # i-th ghost is definately in jail
                        self.particles[index] = self.getParticleWithGhostInJail(particle, i) # particle should have ghost i in jail
        else: # resample
            self.particles = util.nSample(likelihoods.values(), likelihoods.keys(), self.numParticles)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                newParticle[i] = util.sample(newPosDist) # i-th ghost should move to random position (sampled from given distribution) 
